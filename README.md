...
A corpus annotation project built on scraped Brandeis University course syllabi. We scrape syllabi from Moodle, then annotate them by highlighting spans of text that correspond to common student-relevant topics (grading, attendance, accommodations, etc.).
...
Which semesters to scrape is also configured in `config.py` — no need to change unless you're a group member adjusting the data scope.
...
The script will print progress as it crawls and downloads. Output goes to the `syllabi/` folder. The scraper recursively crawls the full semester → school → department → subject hierarchy, downloads all available syllabus files, and deduplicates across semesters by course title and instructor name.

**Important notes about scraping:**

- **Cookie expiration**: Your `MoodleSession` cookie will expire after some hours. If the script reports login errors, grab a fresh cookie from your browser.
- **Be respectful**: The default delay is 0.5 seconds between requests. Don't lower this — Moodle is a shared resource for the whole university.
...
In Label Studio, click **Label All Tasks** to start. For each syllabus, highlight spans of text and assign labels from the toolbar (ACCOM, ATTEND, GRADE, etc.). Click **Submit** after each document.
...
| ASSIGN | Assignments / Requirements | Homework, projects, exams, deliverables |
| DEADLINE | Deadlines / Late Work | Due dates, late penalties, extensions, make-ups |
...
| COMM | Communication / Office Hours | Contacting instructors, office hours, email policy |
...
These labels may change as we iterate on the guidelines. To update them, edit `label_studio_config.xml` and the annotation guidelines. The next time someone runs `python annotate.py start`, the project will use the updated config.
...
- If `annotate.py start` says your token was rejected, go to Account & Settings > Personal Access Token, create a new one, and run `python annotate.py setup` again.